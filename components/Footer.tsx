'use client';

import Link from 'next/link';
import NewsletterSignup from './NewsletterSignup';

// ============================================
// FOOTER COMPONENT
// ============================================

export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-gray-900 text-gray-300">
      {/* Main Footer */}
      <div className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Brand Column */}
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-2 mb-4">
              <span className="text-3xl">🥃</span>
              <span className="text-xl font-bold text-white">CravBarrels</span>
            </Link>
            <p className="text-gray-400 mb-6">
              Your ultimate guide to discovering, learning, and enjoying the world's finest spirits.
            </p>
            
            {/* Social Links */}
            <div className="flex gap-4">
              <a
                href="https://twitter.com/cravbarrels"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-amber-600 transition-colors"
                aria-label="Twitter"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href="https://instagram.com/cravbarrels"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-amber-600 transition-colors"
                aria-label="Instagram"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="https://facebook.com/cravbarrels"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-amber-600 transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href="https://youtube.com/@cravbarrels"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-amber-600 transition-colors"
                aria-label="YouTube"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="text-white font-bold mb-4">Explore</h4>
            <ul className="space-y-3">
              <li>
                <Link href="/explore" className="hover:text-amber-500 transition-colors">
                  Browse Spirits
                </Link>
              </li>
              <li>
                <Link href="/cocktails/genius" className="hover:text-amber-500 transition-colors">
                  Cocktail Genius
                </Link>
              </li>
              <li>
                <Link href="/tasting" className="hover:text-amber-500 transition-colors">
                  Virtual Tasting Room
                </Link>
              </li>
              <li>
                <Link href="/rewards" className="hover:text-amber-500 transition-colors">
                  Rewards Hub
                </Link>
              </li>
              <li>
                <Link href="/marketplace" className="hover:text-amber-500 transition-colors">
                  Marketplace
                </Link>
              </li>
              <li>
                <Link href="/games" className="hover:text-amber-500 transition-colors">
                  Games & Trivia
                </Link>
              </li>
            </ul>
          </div>

          {/* Learn */}
          <div>
            <h4 className="text-white font-bold mb-4">Learn</h4>
            <ul className="space-y-3">
              <li>
                <Link href="/learn/whiskey-101" className="hover:text-amber-500 transition-colors">
                  Whiskey 101
                </Link>
              </li>
              <li>
                <Link href="/learn/tasting-guide" className="hover:text-amber-500 transition-colors">
                  Tasting Guide
                </Link>
              </li>
              <li>
                <Link href="/learn/distilleries" className="hover:text-amber-500 transition-colors">
                  Distillery Directory
                </Link>
              </li>
              <li>
                <Link href="/learn/cocktail-basics" className="hover:text-amber-500 transition-colors">
                  Cocktail Basics
                </Link>
              </li>
              <li>
                <Link href="/blog" className="hover:text-amber-500 transition-colors">
                  Blog
                </Link>
              </li>
            </ul>
          </div>

          {/* Newsletter */}
          <div>
            <NewsletterSignup
              variant="footer"
              title="Join the Community"
              subtitle="Get weekly picks, exclusive deals, and tasting tips."
              source="footer"
            />
          </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
              <span>© {currentYear} CravBarrels. All rights reserved.</span>
              <span className="hidden md:inline">•</span>
              <span>A CR AudioViz AI Product</span>
            </div>
            
            <div className="flex flex-wrap items-center gap-6 text-sm">
              <Link href="/privacy" className="text-gray-500 hover:text-amber-500 transition-colors">
                Privacy Policy
              </Link>
              <Link href="/terms" className="text-gray-500 hover:text-amber-500 transition-colors">
                Terms of Service
              </Link>
              <Link href="/contact" className="text-gray-500 hover:text-amber-500 transition-colors">
                Contact Us
              </Link>
              <Link href="/maker/dashboard" className="text-gray-500 hover:text-amber-500 transition-colors">
                For Makers
              </Link>
            </div>
          </div>

          {/* Age Verification Notice */}
          <div className="mt-6 pt-6 border-t border-gray-800 text-center">
            <p className="text-xs text-gray-600">
              🔞 You must be of legal drinking age in your country to use this site. 
              Please drink responsibly. Don't drink and drive.
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
}
